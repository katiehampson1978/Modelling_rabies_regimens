...
for (i in 1:length(regimens)){
  ...
  # 0.5ml vials
  costPRDA[i,] = costs(overhead, material1, vial.cost, patients, as.data.frame(vuse0.5[i])[,3], unlist(regimens[i]))/DA
  costPRDAuci[i,] = costs(overhead, material1, vial.cost, patients, as.data.frame(vuse0.5[i])[,1], unlist(regimens[i]))/DA
  costPRDAlci[i,] = costs(overhead, material1, vial.cost, patients, as.data.frame(vuse0.5[i])[,5], unlist(regimens[i]))/DA

  # 1ml vials
  costPRDA1[i,] = costs(overhead, material1, vial.cost, patients, as.data.frame(vuse1[i])[,3], unlist(regimens[i]))/DA
  costPRDAuci1[i,] = costs(overhead, material1, vial.cost, patients, as.data.frame(vuse1[i])[,1], unlist(regimens[i]))/DA
  costPRDAlci1[i,] = costs(overhead, material1, vial.cost, patients, as.data.frame(vuse1[i])[,5], unlist(regimens[i]))/DA

  # INSULIN SYRINGES
  # 0.5ml vials
  costPRDAinsulin[i,] = costs(overhead, material2, vial.cost, patients, as.data.frame(vuse0.5[i])[,3], unlist(regimens[i]))/DA
  costPRDAuciinsulin[i,] = costs(overhead, material2, vial.cost, patients, as.data.frame(vuse0.5[i])[,1], unlist(regimens[i]))/DA
  costPRDAlciinsulin[i,] = costs(overhead, material2, vial.cost, patients, as.data.frame(vuse0.5[i])[,5], unlist(regimens[i]))/DA

  # 1ml vials
  costPRDA1insulin[i,] = costs(overhead, material2, vial.cost, patients, as.data.frame(vuse1[i])[,3], unlist(regimens[i]))/DA
  costPRDAuci1insulin[i,] = costs(overhead, material2, vial.cost, patients, as.data.frame(vuse1[i])[,1], unlist(regimens[i]))/DA
  costPRDAlci1insulin[i,] = costs(overhead, material2, vial.cost, patients, as.data.frame(vuse1[i])[,5], unlist(regimens[i]))/DA

  # MARY WARRELL
  # 0.5ml vials - some waste
  costPRDAwaste[i,] = costs(overhead, material1, vial.cost, patients, as.data.frame(vuse0.5waste[i])[,3], unlist(regimens[i]))/DA
  costPRDAwasteuci[i,] = costs(overhead, material1, vial.cost, patients, as.data.frame(vuse0.5waste[i])[,1], unlist(regimens[i]))/DA
  costPRDAwastelci[i,] = costs(overhead, material1, vial.cost, patients, as.data.frame(vuse0.5waste[i])[,5], unlist(regimens[i]))/DA

  # 1ml vials - some waste  - some waste - useful for MW regimens
  costPRDAwaste1[i,] = costs(overhead, material1, vial.cost, patients, as.data.frame(vuse1waste[i])[,3], unlist(regimens[i]))/DA
  costPRDAwasteuci1[i,] = costs(overhead, material1, vial.cost, patients, as.data.frame(vuse1waste[i])[,1], unlist(regimens[i]))/DA
  costPRDAwastelci1[i,] = costs(overhead, material1, vial.cost, patients, as.data.frame(vuse1waste[i])[,5], unlist(regimens[i]))/DA
}


costPRDA_4site0.1ml=costPRDAuci_4site0.1ml=costPRDAlci_4site0.1ml=matrix(NA, nrow=length(regimens), ncol=length(patients))
costPRDA_4site0.1ml = costs(overhead, material1, vial.cost, patients, as.data.frame(vials)[,3], unlist(regimens[4]))/DA
costPRDAuci_4site0.1ml = costs(overhead, material1, vial.cost, patients, as.data.frame(vials)[,1], unlist(regimens[4]))/DA
costPRDAlci_4site0.1ml = costs(overhead, material1, vial.cost, patients, as.data.frame(vials)[,5], unlist(regimens[4]))/DA


# Compare for IPC vs ID4
# vuse1[[7]]
costs(overhead, material2, vial.cost, patients, vuse1[[7]][,3], unlist(regimens[7]))/DA

# vials
costs(overhead, material1, vial.cost, patients, vials[,3], unlist(regimens[4]))/DA
...
# FOR PRESENTATION
# postscript(file="figs/costs.eps", width=4.4, height=4, horizontal=F)
par(cex=0.5, lwd=0.5, plt=c(0.1, 0.5, 0.1, 0.9), mgp=c(1.2,0.2,0), tck=-0.01)
plot(patients, rep(NA, length(patients)), ylim=c(0,250), type="l", lwd=2, log="x", axes=FALSE,
     xlab="Monthly clinic throughput (new bite patients)",
     ylab="Direct medical costs (in USD) per rabies death averted")
...
# WeekIM (dark blue)
polygon(c(patients, sort(patients, decreasing=T)),
        c(costPRDAuci[8,], costPRDAlci[8,length(patients):1]),
        border=NA, col="blue")
lines(patients, costPRDA[8,], col="dark blue", lwd=2)
text(100, 175, "1-week IM", col="dark blue")

# weekIM2 (light blue)
polygon(c(patients, sort(patients, decreasing=T)),
        c(costPRDAuci[9,], costPRDAlci[9,length(patients):1]),
        border=NA, col="light blue")
lines(patients, costPRDA[9,], col="blue", lwd=2)
text(100, 150, "2-visit IM", col="blue")
...
# ID4 (orange), - normal syringes (waste)
polygon(c(patients, sort(patients, decreasing=T)),
...
# TRC4 (dark red)
polygon(c(patients, sort(patients, decreasing=T)),
        c(costPRDAuci[1,], costPRDAlci[1,length(patients):1]),
        border=NA, col="red")
lines(patients, costPRDA[1,], col="dark red", lwd=2)

# IPC (light red) - (no waste)
polygon(c(patients, sort(patients, decreasing=T)),
        c(costPRDAuci[7,], costPRDAlci[7,length(patients):1]),
        border=NA, col="pink")
lines(patients, costPRDA[7,], col="indianred1", lwd=2)

# weekID (red)
polygon(c(patients, sort(patients, decreasing=T)),
        c(costPRDAuci[6,], costPRDAlci[6,length(patients):1]),
        border=NA, col="indianred1")
lines(patients, costPRDA[6,], col="dark red", lwd=2)
dev.off()


###########################################################################
# COSTS OF CURRENT (IM vs ID)
postscript(file="figs/costs_current.eps", width=4.4, height=4, horizontal=F)
par(cex=0.7, new=TRUE, lwd=0.5, plt=c(0.35, 0.95, 0.1, 0.9), mgp=c(1.2,0.2,0), tck=-0.01)
plot(patients, rep(NA, length(patients)), ylim=c(0,250), type="l", lwd=2, log="x", axes=FALSE,
     xlab="Monthly clinic throughput (new bite patients)",
     ylab="Direct medical costs (in USD) per rabies death averted")
axis(2, lwd=0.5); axis(1, lwd=0.5)
...
polygon(c(patients, sort(patients, decreasing=T)),
        ...
        border=NA, col="red")
lines(patients, costPRDAwaste[1,], col="dark red", lwd=2)
text(150, 150, "Updated TRC 0.5mL", col="dark red")
...
text(150, 95, "Updated TRC 1mL", col="dark red")
dev.off()

###########################################################################
# COSTS OF IM - CURRENT AND NEW (compared to ID)
postscript(file="figs/costs_IM.eps", width=4.4, height=4, horizontal=F)
#par(cex=0.5, lwd=0.5, plt=c(0.1, 0.5, 0.1, 0.9), mgp=c(1.2,0.2,0), tck=-0.01)
par(cex=0.7, new=TRUE, lwd=0.5, plt=c(0.35, 0.95, 0.1, 0.9), mgp=c(1.2,0.2,0), tck=-0.01)
plot(patients, rep(NA, length(patients)), ylim=c(0,250), type="l", lwd=2, log="x", axes=FALSE,
     xlab="Monthly clinic throughput (new bite patients)",
     ylab="Direct medical costs (in USD) per rabies death averted")
axis(2, lwd=0.5); axis(1, lwd=0.5)
text(30, 250, "0.5mL or 1mL vials", font=2)
...
# ID -  TRC4 (dark red)
lines(patients, costPRDA1insulin[1,], col="dark red", lwd=1, lty=2)
lines(patients, costPRDAinsulin[1,], col="dark red", lwd=1, lty=2)
text(100, 100, "Updated TRC 0.5mL", col="dark red")
text(100, 65, "Updated TRC 1mL", col="dark red")

# lines(patients, costPRDA1insulin[7,], col="indianred1", lwd=2)
# lines(patients, costPRDAinsulin[7,], col="indianred1", lwd=2)
# lines(patients, costPRDAwaste1[7,], col="indianred1", lwd=2)
# lines(patients, costPRDAwaste[7,], col="indianred1", lwd=2)
dev.off()

postscript(file="figs/costs_IM_noID.eps", width=4.4, height=4, horizontal=F)
#par(cex=0.5, lwd=0.5, plt=c(0.1, 0.5, 0.1, 0.9), mgp=c(1.2,0.2,0), tck=-0.01)
par(cex=0.7, new=TRUE, lwd=0.5, plt=c(0.35, 0.95, 0.1, 0.9), mgp=c(1.2,0.2,0), tck=-0.01)
plot(patients, rep(NA, length(patients)), ylim=c(0,250), type="l", lwd=2, log="x", axes=FALSE,
     xlab="Monthly clinic throughput (new bite patients)",
...
#
# # ID -  TRC4 (dark red)
# lines(patients, costPRDAwaste[1,], col="dark red", lwd=1, lty=2)
# lines(patients, costPRDAwaste1[1,], col="dark red", lwd=1, lty=2)
# text(100, 120, "Updated TRC 0.5mL", col="dark red")
# text(100, 75, "Updated TRC 1mL", col="dark red")
dev.off()



# COSTS OF ID CURRENT AND NEW (compared to IM)
postscript(file="figs/costs_ID.eps", width=4.4, height=4, horizontal=F)
par(cex=0.5, lwd=0.5, plt=c(0.1, 0.5, 0.1, 0.9), mgp=c(1.2,0.2,0), tck=-0.01)
plot(patients, rep(NA, length(patients)), ylim=c(0,250), type="l", lwd=2, log="x", axes=FALSE,
     xlab="Monthly clinic throughput (new bite patients)",
     ylab="Direct medical costs (in USD) per rabies death averted")
...
# IM
# ZAGREB or ESSEN
lines(patients, costPRDA[3,], col="black", lwd=1, lty=2)
text(150, 225, "IM")
...
polygon(c(patients, sort(patients, decreasing=T)),
        ...
        border=NA, col="red")
lines(patients, costPRDAwaste[1,], col="dark red", lwd=2)
text(150, 140, "Updated TRC", col="dark red")
...
#         border=NA, col="indianred1")
# lines(patients, costPRDAwaste[6,], col="dark red", lwd=2)
# text(150, 180, "1 week ID", col="dark red")
...
polygon(c(patients, sort(patients, decreasing=T)),
        ...
        border=NA, col="tan1")
lines(patients, costPRDAwaste[4,], col="darkorange2", lwd=2)
text(150, 120, "4-site ID", col="darkorange2")
...
# IPC (light red) - (no waste)
polygon(c(patients, sort(patients, decreasing=T)),
        c(costPRDAuciinsulin[7,], costPRDAlciinsulin[7,length(patients):1]),
        border=NA, col="pink")
lines(patients, costPRDAinsulin[7,], col="indianred1", lwd=2)
text(150, 95, "IPC", col="indianred1")

############## 1 mL vials
par(cex=0.5, new=TRUE, lwd=0.5, plt=c(0.55, 0.95, 0.1, 0.9), mgp=c(1.2,0.2,0), tck=-0.01)
plot(patients, rep(NA, length(patients)), ylim=c(0,250), type="l", lwd=2, log="x", axes=FALSE,
     xlab="Monthly clinic throughput (new bite patients)",
     ylab="")
axis(2, lwd=0.5); axis(1, lwd=0.5)
...
#         border=NA, col="tan1")
# lines(patients, costPRDA_4site0.1ml, col="darkorange2", lwd=2, lty=2)
...
########################################################################
# COMPARE ID - 1CC vs INSULIN syringes - 1mL
postscript(file="figs/costs_IDsyringe.eps", width=4.4, height=4, horizontal=F)
par(cex=0.7, new=TRUE, lwd=0.5, plt=c(0.35, 0.95, 0.1, 0.9), mgp=c(1.2,0.2,0), tck=-0.01)
plot(patients, rep(NA, length(patients)), ylim=c(0,250), type="l", lwd=2, log="x", axes=FALSE,
     xlab="Monthly clinic throughput (new bite patients)",
     ylab="Direct medical costs (in USD) per rabies death averted")
...
polygon(c(patients, sort(patients, decreasing=T)),
        c(costPRDAuci1insulin[1,], costPRDAlci1insulin[1,length(patients):1]),
        border=NA, col="red")
lines(patients, costPRDA1insulin[1,], col="dark red", lwd=2, lty=2)
...
lines(patients, costPRDAwaste1[7,], col="indianred1", lwd=2)
text(150, 45, "IPC 1mL", col="indianred1")
...
lines(patients, costPRDA1insulin[7,], col="indianred1", lwd=2, lty=2)
lines(patients, costPRDAwaste1[7,], col="indianred1", lwd=2)
text(150, 45, "Insulin syringe", col="indianred1")
text(150, 75, "Standard syringe", col="indianred1")
dev.off()
...
for(i in 1:length(regimens)){
  vuse250[i] = vuse1[[i]][2,3] # for each regimen median vials used for 5 patients/ month
  vuse1000[i] = vuse1[[i]][4,3] # for 20 patients/ month
...
names(regimens)[c(2,8,1,4,7)]
bars = c("Essen", "1-week IM", "TRC", "4-site ID", "IPC")

# regimens - essen4, weekIM1, UTRC, ID4, IPC c(2,8,1,4,7)
postscript(file="figs/patients_treated.eps", width=10, height=4, horizontal=F)
par(cex=0.65, lwd=0.5, plt=c(0.05, 0.35, 0.2, 0.9), mgp=c(1.2,0.2,0), tck=-0.01)
bp=barplot(patients250[c(2,8,1,4,7)],
           axisnames = FALSE,
           main="250 vials available",
           col=c("dark grey", "dark blue", "red", "indianred1", "pink"),
...
bp=barplot(patients1000[c(2,8,1,4,7)],
           axisnames = FALSE,
...
par(cex=0.65, new=TRUE, lwd=0.5, plt=c(0.7, 1, 0.2, 0.9), mgp=c(1.2,0.2,0), tck=-0.01)
bp=barplot(patients3500[c(2,8,1,4,7)],
        axisnames = FALSE,
        main="3500 vials available",
        col=c("dark grey", "dark blue", "red", "indianred1", "pink"),
        ylim=c(0,7000))
text(bp, par("usr")[3], labels = bars, srt = 60, adj = 1, xpd = TRUE, cex=1.2)
dev.off()
